...
for i in range(1, n + 1):
    data = int(input())  # 수열은 그때그때 입력을 받는다.
    while count <= data:  # 입력받은 수열 값에 도달할 때까지 스택에 삽입
        stack.append(count)
        result.append("+")
        count += 1  # 스택에 들어갈 수 1씩 증가
    if stack[-1] == data:  # 스택 최상단의 값이 입력받은 수열 값과 같을 경우
        stack.pop()
        result.append("-")
    else:  # 입력받은 수열 값과 스택 최상단의 값이 다른 경우 -> 다른 수열이 됨
        print("No")
        exit(0)
